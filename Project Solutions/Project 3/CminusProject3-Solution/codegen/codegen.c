...
void addIdToSymtab(DNode node, AddIdStructPtr data)
{
    ...
    data->offset += (int)SymGetFieldByIndex(data->symtab, symIndex, SYMTAB_SIZE_FIELD);
}

int emitComputeVariableAddressFromArray(DList instList, SymTable symtab, int varIndex, int arrayOffset)
{
    ...
    char* regNameOfArrayOffset = (char*)SymGetFieldByIndex(symtab, arrayOffset, SYM_NAME_FIELD);
    ...
    int regMultIndex = getFreeIntegerRegisterIndex(symtab);
    char* regMultName = (char*)SymGetFieldByIndex(symtab, regMultIndex, SYM_NAME_FIELD);

    inst = nssave(3, "\tli ", regMultName, ", 4");
    dlinkAppend(instList, dlinkNodeAlloc(inst));

    inst = nssave(4, "\tmult ", regMultName, ", ", regNameOfArrayOffset);
    dlinkAppend(instList, dlinkNodeAlloc(inst));

    inst = nssave(2, "\tmflo ", regMultName);
    dlinkAppend(instList, dlinkNodeAlloc(inst));

    inst = nssave(6, "\tadd ", regName, ",", regName, ", ", regMultName);
    dlinkAppend(instList, dlinkNodeAlloc(inst));

    freeIntegerRegister((int)SymGetFieldByIndex(symtab, regMultIndex, SYMTAB_REGISTER_INDEX_FIELD));
    freeIntegerRegister((int)SymGetFieldByIndex(symtab, arrayOffset, SYMTAB_REGISTER_INDEX_FIELD));
...
static void createLabel(char label[50])
{
    static int labelCount = 0;
    snprintf(label, 19, "label%d", labelCount);
	labelCount++;
}
...
int emitIfTest(DList instList, SymTable symtab, int regIndex)
{
    ...
    char* newRegName = SymGetFieldByIndex(symtab, newRegIndex, SYM_NAME_FIELD);

    char* inst = nssave(3, "\tli ", newRegName, ", 0");
    dlinkAppend(instList, dlinkNodeAlloc(inst));

    inst = nssave(6, "\tbeq ", regName, ", ", newRegName, ", ", label);
    dlinkAppend(instList, dlinkNodeAlloc(inst));
    freeIntegerRegister((int)SymGetFieldByIndex(symtab, regIndex, SYMTAB_REGISTER_INDEX_FIELD));
...
int emitWhileLoopTest(DList instList, SymTable symtab, int regIndex)
{
    ...
    char* inst = nssave(4, "\tli ", newRegName, ", ", "1");
    dlinkAppend(instList, dlinkNodeAlloc(inst));

    inst = nssave(6, "\tbne ", regName, ", ", newRegName, ", ", label);
    dlinkAppend(instList, dlinkNodeAlloc(inst));
    freeIntegerRegister((int)SymGetFieldByIndex(symtab, regIndex, SYMTAB_REGISTER_INDEX_FIELD));
...
int emitWhileLoopEnd(DList instList, SymTable symtab)
{
    char label[50];
    createLabel(label);
